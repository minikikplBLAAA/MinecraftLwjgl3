...
public class RenderFish extends Render
{
            ...
            float f, float f1)
    {
        ...
        if(entityfish.angler != null)
        {
            ...
            float f11 = ((entityfish.angler.prevRotationPitch + (entityfish.angler.rotationPitch - entityfish.angler.prevRotationPitch) * f1) * 3.141593F) / 180F;
            double d3 = MathHelper.sin(f9);
            double d5 = MathHelper.cos(f9);
            double d7 = MathHelper.sin(f11);
            double d8 = MathHelper.cos(f11);
            double d9 = (entityfish.angler.prevPosX + (entityfish.angler.posX - entityfish.angler.prevPosX) * (double)f1) - d5 * 0.69999999999999996D - d3 * 0.5D * d8;
            double d10 = (entityfish.angler.prevPosY + (entityfish.angler.posY - entityfish.angler.prevPosY) * (double)f1) - d7 * 0.5D;
            double d11 = ((entityfish.angler.prevPosZ + (entityfish.angler.posZ - entityfish.angler.prevPosZ) * (double)f1) - d3 * 0.69999999999999996D) + d5 * 0.5D * d8;
            if(renderManager.options.thirdPersonView)
            {
                ...
                d9 = (entityfish.angler.prevPosX + (entityfish.angler.posX - entityfish.angler.prevPosX) * (double)f1) - d6 * 0.34999999999999998D - d4 * 0.84999999999999998D;
                d10 = (entityfish.angler.prevPosY + (entityfish.angler.posY - entityfish.angler.prevPosY) * (double)f1) - 0.45000000000000001D;
                d11 = ((entityfish.angler.prevPosZ + (entityfish.angler.posZ - entityfish.angler.prevPosZ) * (double)f1) - d4 * 0.34999999999999998D) + d6 * 0.84999999999999998D;
            }
