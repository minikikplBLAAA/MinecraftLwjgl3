    ...
    implements IWorldAccess
{
    ...
    public RenderGlobal(Minecraft minecraft, RenderEngine renderengine)
    {
        ...
        // ZAMIAST: field_1436_w = minecraft.func_6251_l().checkARBOcclusion();
        field_1436_w = org.lwjgl.opengl.GL.getCapabilities().GL_ARB_occlusion_query;
        if(field_1436_w)
        {
            field_1456_c.clear();
        ...
        // GL11.glNewList(field_1433_z, 4864 /*GL_COMPILE*/); // Deprecated in OpenGL 4.6
        byte byte1 = 64;
        int i = 256 / byte1 + 2;
        float f = 16F;
        ...
        // GL11.glNewList(field_1432_A, 4864 /*GL_COMPILE*/); // Deprecated in OpenGL 4.6
        f = -16F;
        tessellator.startDrawingQuads();
    ...
    public void loadRenderers()
    {
        ...
        if(worldObj != null)
        {
            ...
            {
                func_956_b(MathHelper.floor_double(((Entity) (entityliving)).posX), MathHelper.floor_double(((Entity) (entityliving)).posY), MathHelper.floor_double(((Entity) (entityliving)).posZ));
    ...
    public void func_951_a(Vec3D vec3d, ICamera icamera, float f)
    {
        ...
        RenderManager.instance.func_22187_a(worldObj, renderEngine, mc.fontRenderer, mc.field_22009_h, mc.gameSettings, f);
        field_1423_J = 0;
        field_1422_K = 0;
        field_1421_L = 0;
        EntityLiving entityliving = mc.field_22009_h;
        ...
        for(int i = 0; i < list.size(); i++)
        {
            ...
            if(entity.isInRangeToRenderVec3D(vec3d) && icamera.isBoundingBoxInFrustum(entity.boundingBox) && (entity != mc.field_22009_h || mc.gameSettings.thirdPersonView || mc.field_22009_h.func_22051_K()) && worldObj.blockExists(MathHelper.floor_double(entity.posX), MathHelper.floor_double(entity.posY), MathHelper.floor_double(entity.posZ)))
            {
                field_1422_K++;
    ...
    public int func_943_a(EntityLiving entityliving, int i, double d)
    {
        ...
        if(d4 * d4 + d5 * d5 + d6 * d6 > 16D)
        {
            ...
            func_956_b(MathHelper.floor_double(entityliving.posX), MathHelper.floor_double(entityliving.posY), MathHelper.floor_double(entityliving.posZ));
            Arrays.sort(sortedWorldRenderers, new EntitySorter(entityliving));
        }
        int k = 0;
        if(field_1436_w && !mc.gameSettings.anaglyph && i == 0)
        {
            ...
            do
            {
                ...
                func_962_a(byte0, i1);
                // GL11.glPushMatrix(); // Deprecated in OpenGL 4.6
                float f = 0.0F;
                float f1 = 0.0F;
                float f2 = 0.0F;
                for(int k1 = byte0; k1 < i1; k1++)
                {
                    ...
                    ARBOcclusionQuery.glBeginQueryARB(35092 /*GL_SAMPLES_PASSED_ARB*/, sortedWorldRenderers[k1].field_1732_z);
    ...
    private void func_962_a(int i, int j)
    {
        for(int k = i; k < j; k++)
        {
            ...
            field_1456_c.clear();
            ARBOcclusionQuery.glGetQueryObjectuiARB(sortedWorldRenderers[k].field_1732_z, 34919 /*GL_QUERY_RESULT_AVAILABLE_ARB*/);
            if(ARBOcclusionQuery.glGetQueryObjectuiARB(sortedWorldRenderers[k].field_1732_z, 34919 /*GL_QUERY_RESULT_AVAILABLE_ARB*/) != 0)
            {
                sortedWorldRenderers[k].isWaitingOnOcclusionQuery = false;
                sortedWorldRenderers[k].isVisible = ARBOcclusionQuery.glGetQueryObjectuiARB(sortedWorldRenderers[k].field_1732_z, 34918 /*GL_QUERY_RESULT_ARB*/) != 0;
            }
        }
    ...
    private int func_952_a(int i, int j, int k, double d)
    {
        ...
        for(int i1 = i; i1 < j; i1++)
        {
            if(k == 0)
            {
                ...
                if(field_1436_w && !sortedWorldRenderers[i1].isVisible)
                {
                    field_1418_O++;
                } else
                {
            ...
            if(sortedWorldRenderers[i1].skipRenderPass[k] || !sortedWorldRenderers[i1].isInFrustum || !sortedWorldRenderers[i1].isVisible)
            {
                continue;
            }
    ...
    public void func_4141_b(float f)
    {
        ...
        float f1 = (float)(mc.field_22009_h.lastTickPosY + (mc.field_22009_h.posY - mc.field_22009_h.lastTickPosY) * (double)f);
        byte byte0 = 32;
        int i = 256 / byte0;
        Tessellator tessellator = Tessellator.instance;
        ...
        double d = mc.field_22009_h.prevPosX + (mc.field_22009_h.posX - mc.field_22009_h.prevPosX) * (double)f + (double)(((float)field_1435_x + f) * 0.03F);
    ...
    public void func_6510_c(float f)
    {
        ...
        float f2 = 12F;
        float f3 = 4F;
        double d = (mc.field_22009_h.prevPosX + (mc.field_22009_h.posX - mc.field_22009_h.prevPosX) * (double)f + (double)(((float)field_1435_x + f) * 0.03F)) / (double)f2;
        double d1 = (mc.field_22009_h.prevPosZ + (mc.field_22009_h.posZ - mc.field_22009_h.prevPosZ) * (double)f) / (double)f2 + 0.33000001311302185D;
    ...
    public boolean updateRenderers(EntityLiving entityliving, boolean flag)
    {
        ...
        for(int j1 = 0; j1 < l; j1++)
        {
            ...
            if(!flag)
            {
                ...
                {
                    int k2;
                    for(k2 = 0; k2 < 3 && (aworldrenderer[k2] == null || rendersorter.func_993_a(aworldrenderer[k2], worldrenderer1) <= 0); k2++) { }
                    if(--k2 <= 0)
                    {
                        continue;
                    }
        ...
        for(int j3 = worldRenderersToUpdate.size(); j2 != j3; j2++)
        {
            ...
            if(worldrenderer4 == null || worldrenderer4 == aworldrenderer[0] || worldrenderer4 == aworldrenderer[1] || worldrenderer4 == aworldrenderer[2])
            {
                continue;
            }
            if(l2 != j2)
