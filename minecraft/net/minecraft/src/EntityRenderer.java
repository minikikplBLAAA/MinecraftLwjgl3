...
public class EntityRenderer
{
    ...
    public void updateRenderer()
    {
        ...
        float f = mc.theWorld.getLightBrightness(MathHelper.floor_double(mc.field_22009_h.posX), MathHelper.floor_double(mc.field_22009_h.posY), MathHelper.floor_double(mc.field_22009_h.posZ));
    ...
    public void getMouseOver(float f)
    {
        ...
        field_1385_k = null;
        float f1 = 1.0F;
        List list = mc.theWorld.getEntitiesWithinAABBExcludingEntity(mc.field_22009_h, mc.field_22009_h.boundingBox.addCoord(vec3d1.xCoord * d, vec3d1.yCoord * d, vec3d1.zCoord * d).expand(f1, f1, f1));
        double d2 = 0.0D;
    ...
    private void setupCameraTransform(float f, int i)
    {
        ...
        // GL11.glMatrixMode(5889 /*GL_PROJECTION*/); // Deprecated in OpenGL 4.6
        // GL11.glLoadIdentity(); // Deprecated in OpenGL 4.6
        float f1 = 0.07F;
        ...
        if(field_21155_l != 1.0D)
        {
            ...
            GLUHelper.gluPerspective(func_914_d(f), (float)mc.displayWidth / (float)mc.displayHeight, 0.05F, farPlaneDistance);
        } else
        {
            GLUHelper.gluPerspective(func_914_d(f), (float)mc.displayWidth / (float)mc.displayHeight, 0.05F, farPlaneDistance);
        }
        // GL11.glMatrixMode(5888 /*GL_MODELVIEW0_ARB*/); // Deprecated in OpenGL 4.6
        // GL11.glLoadIdentity(); // Deprecated in OpenGL 4.6
        if(mc.gameSettings.anaglyph)
        {
    ...
    private void func_4135_b(float f, int i)
    {
        ...
        if(!mc.gameSettings.thirdPersonView && !mc.field_22009_h.func_22051_K() && !mc.gameSettings.field_22277_y)
        {
            itemRenderer.renderItemInFirstPerson(f);
        }
        // GL11.glPopMatrix(); // Deprecated in OpenGL 4.6
        if(!mc.gameSettings.thirdPersonView && !mc.field_22009_h.func_22051_K())
        {
    ...
    {
        if(GLFW.glfwGetWindowAttrib(mc.window, GLFW.GLFW_FOCUSED) == 0)
        {
        ...
        int k = (MouseLwjgl3Helper.getX() * i) / mc.displayWidth;
        int i1 = j - (MouseLwjgl3Helper.getY() * j) / mc.displayHeight - 1;
        if(mc.theWorld != null)
        {
            renderWorld(f);
    ...
    public void renderWorld(float f)
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            ...
            if(mc.gameSettings.fancyGraphics)
            {
                ...
                if(renderglobal != null)
                {
                    l = renderglobal.func_943_a(entityliving, 1, f);
                }
            ...
            if(field_21155_l == 1.0D && (entityliving instanceof EntityPlayer) && mc.objectMouseOver != null && !entityliving.isInsideOfMaterial(Material.water))
            {
    ...
    public void func_905_b()
    {
        ...
        // GL11.glMatrixMode(5889 /*GL_PROJECTION*/); // Deprecated in OpenGL 4.6
        // GL11.glLoadIdentity(); // Deprecated in OpenGL 4.6
        // GL11.glOrtho(0.0D, i, j, 0.0D, 1000D, 3000D); // Deprecated in OpenGL 4.6
        // GL11.glMatrixMode(5888 /*GL_MODELVIEW0_ARB*/); // Deprecated in OpenGL 4.6
        // GL11.glLoadIdentity(); // Deprecated in OpenGL 4.6
        // GL11.glTranslatef(0.0F, 0.0F, -2000F); // Deprecated in OpenGL 4.6
    }
    ...
    {
        EntityLiving entityliving = mc.field_22009_h;
        GL11.glFogfv(2918 /*GL_FOG_COLOR*/, func_908_a(fogColorRed, fogColorGreen, fogColorBlue, 1.0F));
