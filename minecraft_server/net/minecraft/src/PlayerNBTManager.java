    ...
    implements IPlayerFileData, ISaveHandler
{
    ...
    public void readPlayerData(EntityPlayer entityplayer)
    {
        try
        {
            File file = new File(worldFile, (new StringBuilder()).append(entityplayer.username).append(".dat").toString());
            if(file.exists())
            {
                NBTTagCompound nbttagcompound = CompressedStreamTools.func_770_a(new FileInputStream(file));
        ...
        {
            logger.warning((new StringBuilder()).append("Failed to load player data for ").append(entityplayer.username).toString());
        }
    }
