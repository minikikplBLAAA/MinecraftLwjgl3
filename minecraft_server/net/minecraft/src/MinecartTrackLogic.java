...
class MinecartTrackLogic
{
    ...
    private void func_591_b()
    {
        for(int i = 0; i < connectedTracks.size(); i++)
        {
            ...
            {
                connectedTracks.set(i, new ChunkPosition(minecarttracklogic.trackX, minecarttracklogic.trackY, minecarttracklogic.trackZ));
            }
        }
    ...
    private MinecartTrackLogic getMinecartTrackLogic(ChunkPosition chunkposition)
    {
        ...
        {
            return new MinecartTrackLogic(minecartTrack, worldObj, chunkposition.x, chunkposition.y + 1, chunkposition.z);
        }
        if(worldObj.getBlockId(chunkposition.x, chunkposition.y - 1, chunkposition.z) == minecartTrack.blockID)
        {
            return new MinecartTrackLogic(minecartTrack, worldObj, chunkposition.x, chunkposition.y - 1, chunkposition.z);
    ...
    {
        connectedTracks.add(new ChunkPosition(minecarttracklogic.trackX, minecarttracklogic.trackY, minecarttracklogic.trackZ));
    ...
    {
        boolean flag1 = func_592_c(trackX, trackY, trackZ - 1);
        boolean flag2 = func_592_c(trackX, trackY, trackZ + 1);
        boolean flag3 = func_592_c(trackX - 1, trackY, trackZ);
