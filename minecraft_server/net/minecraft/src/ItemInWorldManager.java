package net.minecraft.src;
 


public class ItemInWorldManager
{

    public ItemInWorldManager(World world)
    {
        field_672_d = 0.0F;
        field_671_e = 0;
        field_670_f = 0.0F;
        field_674_b = world;
    }

    public void func_328_a()
    {
        field_22051_j++;
        if(field_22050_k)
        {
            int i = field_22051_j - field_22046_o;
            int j = field_674_b.getBlockId(field_22049_l, field_22048_m, field_22047_n);
            if(j != 0)
            {
                Block block = Block.blocksList[j];
                float f = block.blockStrength(field_675_a) * (float)(i + 1);
                if(f >= 1.0F)
                {
                    field_22050_k = false;
                    func_325_c(field_22049_l, field_22048_m, field_22047_n);
                }
            } else
            {
                field_22050_k = false;
            }
        }
    }

    public void func_324_a(int i, int j, int k)
    {
        field_22055_d = field_22051_j;
        int l = field_674_b.getBlockId(i, j, k);
        if(l > 0)
        {
            Block.blocksList[l].onBlockClicked(field_674_b, i, j, k, field_675_a);
        }
        if(l > 0 && Block.blocksList[l].blockStrength(field_675_a) >= 1.0F)
        {
            func_325_c(i, j, k);
        } else
        {
            field_22054_g = i;
            field_22053_h = j;
            field_22052_i = k;
        }
    }

    public void func_22045_b(int i, int j, int k)
    {
        if(i == field_22054_g && j == field_22053_h && k == field_22052_i)
        {
            int l = field_22051_j - field_22055_d;
            int i1 = field_674_b.getBlockId(i, j, k);
            if(i1 != 0)
            {
                Block block = Block.blocksList[i1];
                float f = block.blockStrength(field_675_a) * (float)(l + 1);
                if(f >= 1.0F)
                {
                    func_325_c(i, j, k);
                } else
                if(!field_22050_k)
                {
                    field_22050_k = true;
                    field_22049_l = i;
                    field_22048_m = j;
                    field_22047_n = k;
                    field_22046_o = field_22055_d;
                }
            }
        }
        field_672_d = 0.0F;
        field_671_e = 0;
    }

    public boolean func_323_b(int i, int j, int k)
    {
        Block block = Block.blocksList[field_674_b.getBlockId(i, j, k)];
        int l = field_674_b.getBlockMetadata(i, j, k);
        boolean flag = field_674_b.setBlockWithNotify(i, j, k, 0);
        if(block != null && flag)
        {
            block.onBlockDestroyedByPlayer(field_674_b, i, j, k, l);
        }
        return flag;
    }

    public boolean func_325_c(int i, int j, int k)
    {
        int l = field_674_b.getBlockId(i, j, k);
        int i1 = field_674_b.getBlockMetadata(i, j, k);
        boolean flag = func_323_b(i, j, k);
        ItemStack itemstack = field_675_a.getCurrentEquippedItem();
        if(itemstack != null)
        {
            itemstack.hitBlock(l, i, j, k);
            if(itemstack.stackSize == 0)
            {
                itemstack.func_577_a(field_675_a);
                field_675_a.destroyCurrentEquippedItem();
            }
        }
        if(flag && field_675_a.canHarvestBlock(Block.blocksList[l]))
        {
            Block.blocksList[l].harvestBlock(field_674_b, i, j, k, i1);
            ((EntityPlayerMP)field_675_a).field_20908_a.sendPacket(new Packet53BlockChange(i, j, k, field_674_b));
        }
        return flag;
    }

    public boolean func_6154_a(EntityPlayer entityplayer, World world, ItemStack itemstack)
    {
        int i = itemstack.stackSize;
        ItemStack itemstack1 = itemstack.useItemRightClick(world, entityplayer);
        if(itemstack1 != itemstack || itemstack1 != null && itemstack1.stackSize != i)
        {
            entityplayer.inventory.mainInventory[entityplayer.inventory.currentItem] = itemstack1;
            if(itemstack1.stackSize == 0)
            {
                entityplayer.inventory.mainInventory[entityplayer.inventory.currentItem] = null;
            }
            return true;
        } else
        {
            return false;
        }
    }

    public boolean func_327_a(EntityPlayer entityplayer, World world, ItemStack itemstack, int i, int j, int k, int l)
    {
        int i1 = world.getBlockId(i, j, k);
        if(i1 > 0 && Block.blocksList[i1].blockActivated(world, i, j, k, entityplayer))
        {
            return true;
        }
        if(itemstack == null)
        {
            return false;
        } else
        {
            return itemstack.useItem(entityplayer, world, i, j, k, l);
        }
    }

    private World field_674_b;
    public EntityPlayer field_675_a;
    private float field_672_d;
    private int field_22055_d;
    private int field_671_e;
    private float field_670_f;
    private int field_22054_g;
    private int field_22053_h;
    private int field_22052_i;
    private int field_22051_j;
    private boolean field_22050_k;
    private int field_22049_l;
    private int field_22048_m;
    private int field_22047_n;
    private int field_22046_o;
}
