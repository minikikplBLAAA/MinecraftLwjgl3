...
public class BlockRedstoneTorch extends BlockTorch
{
    ...
    private boolean checkForBurnout(World world, int i, int j, int k, boolean flag)
    {
        if(flag)
        {
            torchUpdates.add(new RedstoneUpdateInfo(i, j, k, world.func_22080_k()));
        }
        int l = 0;
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        ...
        for(; torchUpdates.size() > 0 && world.func_22080_k() - ((RedstoneUpdateInfo)torchUpdates.get(0)).updateTime > 100L; torchUpdates.remove(0)) { }
        if(torchActive)
        {
            if(flag)
            {
